<template>
  <b-card
    ...
    <template v-if="poll.my_answer > -1">
      ...
      <template v-for="option in poll.answers">
    ...
    <b-form-group label="Please select your response" v-else>
      <b-form-radio-group stacked v-model="selected" plain>
        <b-form-radio v-for="option in poll.answers" :value="option.id" :key="option.id"><strong>{{option.content}}</strong></b-form-radio>
      </b-form-radio-group>
    </b-form-group>
  </b-card>
</template>
...
export default {
  ...
  computed: {
    totalAnswers () {
      var total = 0
      for(var option of this.poll.answers) {
        console.log(option)
        total += option.current_selections
      }
